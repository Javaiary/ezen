'''
    ...
    2. 텐서플로 (TensorFlow)
        - 기계학습 프레임워크(framework)
            - 텐서(tensor)를 Numpy배열처럼 사용할 수 있음
            - 데이터 흐름 그래프(data flow graph)를 사용하여 데이터의 수치연산을 하는 오픈 소스 소프트웨어 프레임워크
            - 넘파이같은 다양한 라이브러리를 묶어놓은 패키지라고도 할 수 있음
        - Tensorflow를 사용하면, GPU연동을 통해 효율적으로 딥러닝 모델을 학습할 수 있음
    ...
    4. 텐서플로 특징 및 장점
        ...
        5) 텐서플로 2.x부터는 데이터를 학습하고 예측하는 과정이 매우 단순해졌음
...
'''
    길이가 100이고 shpae가 (3,)인 1차원 배열을 생성한 후, (100,3) 크기의 2차원 배열로 변환함
'''
