'''
    1. 텐서플로 일반적인 아키텍처
        1) 준비된 데이터르 ㄹ사용하여 모델(모형)을 생성
            - 데이터 준비, 모델 정의, 모델 훈련, 모델 평가
        2) 생성된 모델을 사용하여 분류 및 예측
        3) 사용자에게 웹이나 모방일로 서비스를 배포, 환경도 제공
    ...
    3. 데이터 준비
        ...
        2) 서로 다른 길이의 시퀀스(sequence)를 분할하여 처리
'''
import os
...
dense_layer_2 = Dense(10, activation='relu')(input_layer)
output = Dense(y.shape[1], activation='softmax')(dense_layer_2)

model = Model(inputs=input_layer, outputs  = output)
# categorical_corssentropy의 출력 값은 원-핫 인코딩
