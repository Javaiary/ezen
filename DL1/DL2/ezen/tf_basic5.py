'''
    ...
    8) 텐서의 평균 함수
        ...
        텐서의 차원 줄이기 혹은 늘리기
            ...
            - squeeze()
                - 크기가 1인 차원을 제거함
    9) 자동 미분과 기울기 (radient)
        - Tensorflow 에서는 Gradient
        - '기울기 테이프' 라는 의미를 가짐
        - 중간의 관련 연산들을 테이프에 기록하고,
          역전파를 수행했을 떄 기울기가 계산됨.
'''
import os
import numpy as np
...
print(tf.reduce_sum(a, axis=1))     # 각 행에 대햐여 평균 계산
...
print(tf.argmax(a, axis=1))     # 각 행에 대하여 최대값의 인덱스


print("---------------------------------------------")

# 첫 번째 축에 차원 추가(2차원 => 3차원)
