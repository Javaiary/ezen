    ...
    1. 인공신경망
        ...
        5) 경사 하강법
            - 신경망 훈련의 목표는 최적 파라미터를 찾는 것임
                - 파라미터는 가중치와 편향
            - 최적 파라미터는 손실함수가 최솟값일 때의 파라미터를 뜻함
            ...
            - 손실함수(loss function)
                ...
                - 예측한 값과 실제값의 차이(손실)를 구하는 함수
                    - 손실함수의 값이 작을수록 좋은 모델
                - 평균 제곱 오차 (MSE), 교차 엔트로피(crossentropy)
            ...
            - 경사 하강법 (gradient descent)
                ...
                - 절차
                    ...
                    - 학습률 (learning rage)
                        ...
                        - 학습률에 따라 훈련속도가 달라짐
                            -학습률이 너무 크면 훈련 속도는 빠르지만, 더 빠른 길로 이어지는 지점을 건너뛰어
                             최적 가중치를 찾지 못할 수 있음
                            - 학습률이 너무 작으면 훈련속도가 느려짐
                            - 한 걸음에 얼마만큼 이동할지 결정하는게 학습률임
                        - 하이퍼파라미터이기에 우리가 직접 설정해 주어야 함
                                - 보통 0.1~ 0.0001범위의 값을 사용함
                    - mini batch
                ...
                6) 순전파 & 역전파
                    - forwartd propagation
                        - 신경망에서 입력값이 입력층과 은닉층을 거쳐 출력층에 도달하기 까지의 계산 과정
        ...
        2. 딥러닝 알고리즘 성능 향상
            1) 드롭아웃(dropout)
                ...
                - 훈련 단계에서만 적용함, 검증이나 예측단계에서는 적용하지 않음
            2)  옵티마이저 (optimizer)
                - 신경망의 최적 가중치를 찾아주는 알고리즘
                - 확률적 경사 하강법
                - 모멘텀, Adam, RMSProp, Adagrad
                    - Adam : 딥러닝 모델을 설계할 ㅏ때 가장 많이 사용하는 옵티마이저
'''