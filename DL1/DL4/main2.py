...
class Model(nn.Module):         # 클래스 형태로 구현되는 모델은 nn.Module을 상속받음
    '''
        self : 첫 번째 파라미터로 self지정해야 함, 자기 자신을 가리킴
    ...
    def __init__(self, embedding_size, output_size, layers, p=0.4):
        super().__init__()          # 부모클래스의 생성자 호출 필요
        ...
        for i in layers:            # 모델의 네트워크 계층 구축
            all_layers.append(nn.Linear(input_size, i))     # 선형 계층은 입력 데이터에 선형 변환을 위해서 사용
        ...
        self.layers = nn.Sequential(*all_layers)      # 레이어를 all_layers에 append된 순서대로 적용시킴
    ...
    def forward(self,x_categorical ):                 # 학습 데이터를 입력받아서 연산을 진행
        ...
        x = self.embedding_dropout(x)         # 무작위로 40%감소시킨 것을 다시 x에 할당
        x = self.layers(x)
        return x

'''
    Model 클래스의 객체 생성하기
    객체를 생성하면서 embedding_size, output_size, layers, p=0.4        
                범주형 컬럼의 임베딩크기, 출력 크기, 은닉층의 뉴런, 드롭아웃 을 전달 
    은닉층의 뉴런(layers)의 정의는 여러 크기로 지정하여 테스트해 보는 것이 학습에 도움이 될 것 
'''
model = Model(categorical_embedding_sizes, 4, [200,100,50], p=0.4)
print(model)

'''
    모델을 훈련시키기 전 손실함수와 옵티마이저에 대해 정의
    이 DL4 경우 데이터를 분류해야 하는 것으로 크로스 엔트로피 (cross entropy) 손실함수를 사용함
    옵티마이저는 아담 (Adam) 을 사용함
    
    lr(Learning Rate) : 미분값을 얼마만큼 이동시킬 것인가를 경험함
                        초기값이 크다면 초반에 loss값이 빠르게 줄어들 것임. 나중에 가면 unsderfitting
'''
