...
def plus_ver2(number_01, number_02 =100):
    print(f"number_01= {number_01}, number_02= {number_02}")
...
print()
'''
    *args : 함수를 호출할 때 아규먼트의 갯수를 특정지을 수 없을 때 사용
...
'''
    **kwargs : 함수를 호출할 때 키워드 아규먼트의 갯수를 특정지을 수 없을 때 사용
               불특정한 수의 숫자들은 'kwargs'라는 딕셔너리에 추가됨
'''

def plus_unlimited(*args, **kwargs):
    '''
    *args **kwargs 라는 딕셔너리의 형태로 값을 받는 함수
    전체 합의 결과를 출력합니다.
    '''
...
# plus_unlimited(list_numbers) 오류 :
plus_unlimited(*list_numbers)

'''
    딕셔너리를 함수의 파라미터로 넣어 실행하는 경우
'''
dict_heights = {'ezen_height' : 178, 'ezenyoung_height':180, 'sugiezen_height':170} 
    ...
    *Lambda 함수
        ...
        - lambda가 유용하게 사용되는 대표적 함수(3가지)
            ...
            - reduce() : 앞 2개의 원소를 가지고 연산.
                         연산의 결과가 다음 연산의 입력이 됨
                         따라서 마지막 까지 진행되면 최종 출력은 1 개의 값만 남게 됨
'''
square=lambda x:x**2
print(square(5))
...
# pythagoras 함수를 lambda함수로 구현하시오.
pythagoras_lam = lambda num1, num2 :(num1**2 + num2**2)**0.5
print(pythagoras(4,3))
print(pythagoras_lam(3,4))

'''
    키가 180cm 이상인 사람들의 경우 "키가 정말 큽니다." 라는 메시지가 출력되고,
    그렇지 않은 경우를 "계속 키가 클겂니다. " 라는 메시지로 출력하는 함수를 작성하시오.
    height_high(181)
    height_high(178)
'''
def height_high(height):
    return print('키가 정말 큽니다.') if height>=180 else print('계속 키가 클겁니다.')
...
'''
    ...
    # map(함수, 입력들)
    map(function, *iterables) 형태로 사용 가능함
        - *iterables : 원소를 하나씩 차례로 변환 가능한 object를 의미함(list, str, tuple...)
'''
...
dict_heights = {'ezen_height' : 178, 'ezenyoung_height':180, 'sugiezen_height':170, 'esun_height':180, 'eman_height': 175}
