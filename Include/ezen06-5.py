...
end9w = time.time()
print(f"소요시간 : {end9w - start9w}")

'''
    얼마나 시간이 걸리고 갯수가 몇 개인지 확인하시오.
    - for 문을 이용하여 1부터 1000000까지의 숫자 중에서 3의 배수인 숫자들을 찾고, 몇 개가 있는지 알아보자.
...
if money >= 5000 :
    if taxi :
        money -= 3000
        print("택시로 귀가. 잔액 :", money)
    else:
        print("걸어서 귀가. 잔액 :", money)
elif money >= 2000:
    if bus:
        money -= 1000
        print("버스로 귀가. 잔액 :", money)
    else:
        print("걸어서 귀가. 잔액 :" ,money)
else:
    print("걸어서 귀가. 잔액이 2000원 미만입니다. 잔액 : ", money)

'''
    도진이가 택시에 내려서 걸어서 귀가하던 중,
    집 근처 오락실에서 '철권'게임을 하고 싶어졌습니다.
    회당 500원인 이 게임을 몇 번이난 할 수 있을까요?
    매번 게임을 진행한뒤 도준이가 가지고 있는 잔액과 몇 번 게임을 했는지 횟수를 출력하는 코드를 작성하시오.
